                  ...
                  <tbody>
                    @foreach ( $data_barang as $item )
                        <tr>
                            ...
                            <td>{{ $item->status }}</td>
                            <td>
                                <form class="row d-flex align-items-center" style="width:200px"  action="{{ '/Pengadaan/create/'.$vendor->id.'/'.$item->id.'/'.$pengadaan->id }}" method="POST">
                                    @csrf
                                    @method('POST')
                                    <div class="col-7 p-0">
            ...
            <div class="card recent-sales overflow-auto">
              <div class="card-body">
                ...
                <table class="table">
                  ...
                  <br>
                  <h5 class="card-title">Keranjang Pengadaan </h5>
                <table class="table table-borderless datatable">
                  <thead>
                    <tr>
                      <th scope="col">id</th>
                  ...
                  <tbody>
                    @foreach ( $detail_pengadaan as $item )
                        <tr>
                            ...
                            {{-- <td>{{ $item->status }}</td> --}}
                            <td>
                                <form class="d-inline" action="{{ '/Pengadaan/create/'.$vendor->id.'/'.$item->id.'/'.$pengadaan->id }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-danger btn-sm">Delete</button>
                                </form>
